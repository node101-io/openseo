...
const UPLOADS_DIR = path.join(MOCK_FILECOIN_ROOT, 'uploads', 'htmls');
const STORAGE_FILE = path.join(APP_ROOT, 'src', 'storage', 'filecoin-db.json');
...
export function getFileByCID(cid: string): { cid: string; file: string; uploadedTime: string } | null {
    ...
    const absolutePath = path.join(MOCK_FILECOIN_ROOT, record.file);
