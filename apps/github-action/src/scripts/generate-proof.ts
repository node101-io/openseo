...
async function main() {
    ...
    const path = pathArg || process.env.TEST_HTML_PATH || process.env.HTML_PATH;
    const keywordsRaw = keywordsRawArg || process.env.TEST_KEYWORDS || process.env.KEYWORDS;
    if (!path || !keywordsRaw) {
        console.error('Usage: tsx generate-proof.ts <path> \'["keyword1","keyword2"]\'');
        console.error('Or set env: TEST_HTML_PATH, TEST_KEYWORDS (or HTML_PATH, KEYWORDS)');
        process.exit(1);
    }
    ...
    console.log(JSON.stringify({ ...out, proofOutputFile: PROOF_OUTPUT_FILE }));
}
