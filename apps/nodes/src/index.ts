        ...
        this.pollingInterval = setInterval(async () => {
            ...
            try {
                ...
                if (currentBlock > this.lastProcessedBlock) {
                    ...
                    if (rawLogs.length > 0 && newEvents.length === 0) {
                        console.warn(`[${this.nodeName}] Contract has ${rawLogs.length} log(s) in blocks ${fromBlock}-${currentBlock} but VerificationRequested filter matched 0 (check ABI/event name)`);
                    }
                    console.log(`[${this.nodeName}] VerificationRequested events found: ${newEvents.length} (raw logs from contract: ${rawLogs.length})`);
