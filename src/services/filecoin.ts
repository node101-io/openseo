...
const STORAGE_FILE = path.join(process.cwd(), 'storage', 'filecoin_db.json');
...
function loadStorage(): FileRecord[] {
    try {
        if (fs.existsSync(STORAGE_FILE)) {
            ...
            console.log(`Loaded ${records.length} files from storage`);
            return records;
        }
    } catch (error) {
...
function saveStorage(records: FileRecord[]): void {
    try {
        ...
        console.log(`Saved ${records.length} files to storage`);
    } catch (error) {
...
export function storeFile(existingFilePath: string): { 
    ...
    console.log(`File stored with CID: ${cid} at ${relativePath}`);
