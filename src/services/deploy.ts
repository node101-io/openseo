...
async function main() {
  ...
  // Node private key'lerini al - .env'den veya Hardhat default'larından
  const node1PrivateKey = process.env.NODE1_PRIVATE_KEY || (isLocalhost ? HARDHAT_KEYS.NODE1 : "");
  const node2PrivateKey = process.env.NODE2_PRIVATE_KEY || (isLocalhost ? HARDHAT_KEYS.NODE2 : "");
  const node3PrivateKey = process.env.NODE3_PRIVATE_KEY || (isLocalhost ? HARDHAT_KEYS.NODE3 : "");
  ...
  // Private key'lerden adresleri derive et
