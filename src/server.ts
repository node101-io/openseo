...
app.post('/api/upload', uploadFields, async (req: Request, res: Response) => {
    const files = req.files as { htmlFile?: Express.Multer.File[]; keyword?: Express.Multer.File[] };
    const fileEntry = files?.htmlFile?.[0];
    if (!fileEntry) {
    ...
    try {
        ...
        console.log(`Generating word-score proofs with Merkle tree...`);
        ...
        const statusCode = (error as Error).message?.includes('not present') || (error as Error).message?.includes('Keyword') ? 400 : 500;
